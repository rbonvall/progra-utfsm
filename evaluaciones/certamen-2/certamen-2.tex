...
%      para obtener el país de procedencia de cada cliente.
%
%      %En una dirección de correo electrónico,
%      %el \emph{dominio} es la parte que va después de la arroba,
%      %y el \emph{TLD} es lo que va después del último punto.
%      %Por ejemplo,
%      %en la dirección \texttt{fulano.de.tal@alumnos.usm.cl},
%      %el dominio es \texttt{alumnos.usm.cl},
%      %y el TLD es \texttt{cl}.
%
%      En una dirección de correo electrónico,
%      la parte después del último punto se llama \emph{TLD},
%      e indica en muchos casos el país de procedencia del usuario.
%      Por ejemplo,
%      en la dirección \texttt{fulano.de.tal@alumnos.usm.cl},
%      el TLD es \texttt{cl},
%      e indica que el usuario es de Chile.
%
%      Algunos TLD no están asociados a un país,
%      sino que representan otro tipo de entidades.
%      Estos TLD genéricos son los siguentes:
%      \lstinputlisting[linerange=GENERICOS-FIN\ GENERICOS]{pauta3.py}
%
%      %\begin{enumerate}
%      %  \item Escriba la función \li!obtener_dominios(correos)!
...
%      %  \item Escriba la función \li!contar_tld(correos)!
...
%      %\end{enumerate}
%      %\lstinputlisting[linerange=CASO-FIN\ CASO]{pauta3.py}
%
%      Escriba la función \li!contar_tld(correos)!
%      que reciba como parámetro la lista de correos electrónicos,
%      y retorne un diccionario que asocie a cada TLD
%      la cantidad de veces que aparece en la lista,
%      sin incluir los TLD genéricos:
%      \lstinputlisting[linerange=LISTA-FIN\ LISTA]{pauta3.py}
%      \lstinputlisting[linerange=CASO\ CONTAR-FIN\ CASO\ CONTAR]{pauta3.py}
